...
Fiziksel cihaz açılışı kullanıcı tarafından doğrulandı: macOS 15.6.1, MacBook Air M1 (2020). İlk Finder açılışı genel hata verdi; Gizlilik ve Güvenlik'te “Yine de Aç” görünmedi ve doğrudan çalıştırma `Operation not permitted` döndürdü. Kullanıcı, yalnızca `/Applications/Folio.app` üzerinde `codesign --verify --deep --strict` → başarılıysa `xattr -dr com.apple.quarantine` → başarılıysa `open` zincirinden sonra uygulamanın açıldığını bildirdi. Önceden karantina bayrakları alınmadığından engelin kesin alt nedeni kaydedilmedi. Bu doğrulama ilk açılışla sınırlı; fiziksel Mac'te kütüphane geri yükleme ve PDF etkileşim kabulü henüz bildirilmedi.
